    ...
    setup(props) {
        ...
        return () => (
            <div class="layout-home-browse flex flex-col overflow-hidden">
                <n-h2 class="common-width-inline lg:text-28 sm:text-20 xxs:text-16  line-height-34 m-be-0 p-bs-60 font-600">
                    ...
                    {store.browses.map(item => (
                        <n-carousel-item key={item.keyId} class="relative b-rd-4 overflow-hidden">
                            ...
                            <div class="absolute inset-0 flex flex-col p-24">
                                ...
                                <n-h2 class="m-block-10 text-28 text-white font-600">
                                    <n-ellipsis tooltip={false}>{item.name}</n-ellipsis>
                                </n-h2>
                                <n-text class="h-72 flex text-16 line-height-24 text-white">
                                    <n-ellipsis tooltip={false} line-clamp={3}>
                                        {item.document}
                                    </n-ellipsis>
                                </n-text>
                                <router-link class="m-bs-24 overflow-hidden" to="/about">
                                    <n-text class="flex items-center gap-5 text-18 line-height-24 text-white hover:text-[var(--primary-color)]">
                                        <span>More products</span>
                                        <common-wrapper name="nest-double-right" size={18}></common-wrapper>
                                    </n-text>
                                </router-link>
                            </div>
                        </n-carousel-item>
