...
export async function render(request: Request) {
    ...
    router.push(request.originalUrl)
    ...
    const template = await renderToString(app, renderCtx)
    const links = renderPreloadLinks(renderCtx.modules, {})
    const state = JSON.stringify(store.state.value)
    return { template, links, state }
}
